...
async function ensureDeps() {
  ...
  for (const dep of deps) {
    try { await import(dep) } 
      ...
      try { execSync(`npm install ${dep}`, { stdio: "inherit" }) }
      catch (err) { console.error(`Failed to install ${dep}:`, err.message); process.exit(1) }
    }
  }
...
function restart() {
  try {
    ...
    const child = spawn('node', [absolutePath], { detached: false, stdio: 'inherit', windowsHide: false })
    child.on('exit', code => process.exit(code || 0))
    child.on('error', error => { console.error('Failed to restart:', error.message); process.exit(1) })
  } catch (error) {
...
async function upd8() {
  try {
    ...
    if (!newer) { writelog("failed to grab remote file"); return false }
    const versionMatch = newer.match(/const ver\s*=\s*["'](.*?)["']/)
    if (!versionMatch || !versionMatch[1]) { writelog("couldn't read remote ver"); return false }
...
async function updateChangelog() {
  try {
    ...
    if (!log) { writelog("failed to grab changelog"); return }
    let oldLog = ""
    if (fs.existsSync(files.changelog)) oldLog = fs.readFileSync(files.changelog, "utf8")
...
async function fuckingidk(n, junk, pre = "", bag = []) {
  if (pre.length === n) { bag.push(pre); return }
  for (const x of junk) await fuckingidk(n, junk, pre + x, bag)
  return bag
}
...
async function check(p, u, nam, stuf) {
  ...
  try {
    ...
    if (txt.includes("This user has been banned") || txt.includes("User is banned")) {
      console.log("banned: " + nam)
      fs.appendFileSync(stuf.file, "banned: " + nam + "\n")
      return false
    } else if (txt.includes("This user is not claimed") || txt.includes("The profile you are looking") || txt.includes("This username is available!")) {
